...
def Init():
    ...
    objetos.append(OBJ("Envirioment/MapObjs/tank.obj" , swapyz=True)) #7   
...
def display():
    ...
    #Dibujo de evironment (mapa)
    glPushMatrix()
    glScale(1.2,1.2,1.2)
    objetos[6].render()
    glPopMatrix()

    # Dibujo de 4 tanks (objetos[7]) en las esquinas del DimBoard, a nivel y=0
    corner_positions = [(-DimBoard, 0.0, -DimBoard),
                        (-DimBoard, 0.0, DimBoard),
                        (DimBoard, 0.0, -DimBoard),
                        (DimBoard, 0.0, DimBoard)]

    for (cx, cy, cz) in corner_positions:
        glPushMatrix()
        # Colocar el tank en la esquina
